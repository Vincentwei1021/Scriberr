...
export function OpenClawProfileSettings() {
	...
	const hasProfiles = useMemo(() => profiles.length > 0, [profiles.length]);
	...
	const openEditDialog = (profile: OpenClawProfile) => {
		...
		setForm({
			...
			hook_key: profile.hook_key,
			hook_name: profile.hook_name,
			message: profile.message,
		});
		setSshKeyFileName("已保存密钥（重新选择文件可覆盖）");
		setDialogOpen(true);
	};
	...
	const onSshKeyFileSelected = async (e: ChangeEvent<HTMLInputElement>) => {
		const file = e.target.files?.[0];
		if (!file) return;
		try {
			const keyContent = await file.text();
			setForm((s) => ({ ...s, ssh_key: keyContent }));
	...
	const onSave = async () => {
		setError("");
		if (!form.name.trim() || !form.ip.trim() || !form.ssh_key.trim() || !form.hook_key.trim() || !form.hook_name.trim() || !form.message.trim()) {
				...
				<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
					<div>
						...
						<p className="text-sm text-[var(--text-secondary)] mt-1">
							配置远端机器连接信息，用于发送 SRT 并触发 OpenClaw 总结。
						</p>
					</div>
				...
				<DialogContent className="max-w-[760px] bg-[var(--bg-card)] border border-[var(--border-subtle)]">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div className="space-y-2">
							...
							<Input value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} placeholder="e.g. OpenClaw Prod" />
						</div>
						<div className="space-y-2">
							<label className="text-sm text-[var(--text-secondary)]">IP / Host (supports user@host)</label>
							<Input value={form.ip} onChange={(e) => setForm((s) => ({ ...s, ip: e.target.value }))} placeholder="ubuntu@10.0.0.8" />
						</div>
						<div className="space-y-2 md:col-span-2">
							...
							<div className="space-y-2">
								...
								<p className="text-xs text-[var(--text-secondary)]">
									{sshKeyFileName || "未选择文件"}
								</p>
							</div>
						</div>
						<div className="space-y-2">
							<label className="text-sm text-[var(--text-secondary)]">Hook Key</label>
							<Input value={form.hook_key} onChange={(e) => setForm((s) => ({ ...s, hook_key: e.target.value }))} placeholder="Bearer token value" />
						</div>
						<div className="space-y-2">
							<label className="text-sm text-[var(--text-secondary)]">Hook Name</label>
							<Input value={form.hook_name} onChange={(e) => setForm((s) => ({ ...s, hook_name: e.target.value }))} placeholder="Dashboard" />
						</div>
						<div className="space-y-2 md:col-span-2">
							<label className="text-sm text-[var(--text-secondary)]">Message</label>
							<Textarea value={form.message} onChange={(e) => setForm((s) => ({ ...s, message: e.target.value }))} rows={4} placeholder="Prompt to OpenClaw agent" />
						</div>
					</div>
