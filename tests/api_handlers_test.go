...
func (suite *APIHandlerTestSuite) TestOpenClawProfileManagement() {
	...
	var created models.OpenClawProfile
	...
	var list []models.OpenClawProfile
	...
	updateData := map[string]interface{}{
		...
		"ip":        "ubuntu@10.0.0.9",
		"ssh_key":   "ssh-key-new",
		"hook_key":  "hook-key-new",
		"hook_name": "Dashboard",
		"message":   "Summarize now",
	}
	w = suite.makeAuthenticatedRequest("PUT", fmt.Sprintf("/api/v1/openclaw/profiles/%s", created.ID), updateData, true)
	assert.Equal(suite.T(), 200, w.Code)

	w = suite.makeAuthenticatedRequest("DELETE", fmt.Sprintf("/api/v1/openclaw/profiles/%s", created.ID), nil, true)
	assert.Equal(suite.T(), 200, w.Code)
}
