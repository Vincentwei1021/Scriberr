...
// NewOpenClawServiceWithRunner creates a service with a custom command runner (tests).
...
// SendSRT uploads an SRT file to a remote host via SCP, then triggers OpenClaw hook over SSH.
func (s *OpenClawService) SendSRT(ctx context.Context, profile *models.OpenClawProfile, srtContent string, title string, transcriptionID string) (*OpenClawSendResult, error) {
	...
	localPath, cleanupSRT, err := writeTempSRT(fileBase, srtContent)
	if err != nil {
		return nil, err
	}
	defer cleanupSRT()
...
func writeTempSRT(baseName string, content string) (string, func(), error) {
	...
	_ = baseName // Keep signature explicit and ready for future filename use.
	return file.Name(), cleanup, nil
}
